...
refs <- readDNAStringSet("/root/References/references.fasta")

# make a list to filter out samples we didnt sequence later
samples <- read.csv("/root/References/sampleNames.txt", header = TRUE, sep = "\t")[, 1:2]
...
for (i in 1:148) {
  ...
  WXYs[[i]] <- replaceLetterAt(
    WXYs[[i]],
    1:length(WXYs[[1]]) %in% as.numeric(names(pos)),
    as.character(WXYSNPs[i, names(pos)])
  )
}
wxysamples <- c(
  paste0(samples[grep("^WXY", samples)], "_0"),
  paste0(samples[grep("^WXY", samples)], "_1")
)
wxysamples <- wxysamples[order(wxysamples)]
WXYs <- WXYs[names(WXYs)[names(WXYs) %in% wxysamples]]
writeXStringSet(WXYs, "/root/Results/PHYLO/WXY_Phased.fasta")
...
writeXStringSet(WXYsa, "/root/Results/PHYLO/WXY_ambig.fasta")
...
writeXStringSet(LFYs, "/root/Results/PHYLO/LFY_Phased.fasta")
...
writeXStringSet(LFYsa, "/root/Results/PHYLO/LFY_ambig.fasta")

# write nuclear ambiguous
Nuclearsa <- xscat(LFYsa, WXYsa)
names(Nuclearsa) <- substr(names(LFYsa), 5, length(names(LFYsa)))
writeXStringSet(Nuclearsa, "/root/Results/PHYLO/Nuclear_ambig.fasta")
...
writeXStringSet(matKs, "/root/Results/PHYLO/matK_Phased.fasta")
...
writeXStringSet(matKsa, "/root/Results/PHYLO/matK_ambig.fasta")
...
writeXStringSet(trnLs, "/root/Results/PHYLO/trnL_Phased.fasta")
...
writeXStringSet(trnLsa, "/root/Results/PHYLO/trnL_ambig.fasta")
...
writeXStringSet(psaAs, "/root/Results/PHYLO/psaA_Phased.fasta")
...
writeXStringSet(psaAsa, "/root/Results/PHYLO/psaA_ambig.fasta")
