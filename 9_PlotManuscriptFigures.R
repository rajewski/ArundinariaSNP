# This code is intended to make the final publication quality versions of the figures for this manuscript
...
species <- read.csv("/mnt/References/JTCoords.csv", header = T, stringsAsFactors = F) %>%
  mutate(
    Species = gsub(pattern = "\\.| ", "", Species),
    Species = substr(Species, 1, 4),
    Species = factor(Species, levels = c("Aapp", "Agig", "Atec", "Ahyb", "Unkn"))
  )
...
pdf(file = "/mnt/Results/Manuscript/Figure_1.pdf", height = 6, width = 6)
# png(file="/mnt/Results/Manuscript/Figure_1.png", height=6, width=6, units="in", res=600)
map("state", ArunRange)
...
nucmds <- read.table("/mnt/Results/Clustering/Nuclear.mds",
  header = T,
  stringsAsFactors = F
) %>%
  merge(species,
...
WXYmds <- read.nexus.dist(file = "/mnt/Results/PHYLO/WXY/WXY.dist.nex")
LFYmds <- read.nexus.dist(file = "/mnt/Results/PHYLO/LFY/LFY.dist.nex")
Plasmds <- read.nexus.dist(file = "/mnt/Results/PHYLO/plastid_nomissing/Plastid_NoMissing.dist.nex")
Allmds <- read.nexus.dist(file = "/mnt/Results/PHYLO/concatenated/TESTconatenated.dist")
...
ggsave2(filename = "/mnt/Results/Manuscript/Figure_2.pdf", height = 6, width = 9)
ggsave2(filename = "/mnt/Results/Manuscript/Figure_2.png", height = 6, width = 9)

plot_grid(LFYplot, legend, nrow = 2, rel_heights = c(1, 0.1))
ggsave2(filename = "/mnt/Results/Manuscript/Supplemental_2_MDS_LFY.pdf", height = 7, width = 9)
ggsave2(filename = "/mnt/Results/Manuscript/Supplemental_2_MDS_LFY.png", height = 7, width = 9)
...
# plastid data
SplitsPlas <- read.nexus.networx("/mnt/Results/PHYLO/plastid_nomissing/Plastid_NoMissing.splits.nex")
SplitsPlas <- TipClean(SplitsPlas)
SplitsPlasAcro <- TipClean(SplitsPlas, acronym = T)

# LFY data
SplitsLFY <- read.nexus.networx("/mnt/Results/PHYLO/LFY/LFY.splits.nex")
# Save one unrotated for "tall" plot
SplitsLFY_Tall <- SplitsLFY
# Rotate another for the default (wide) plot
SplitsLFY$.plot$vertices <- cbind(
  SplitsLFY$.plot$vertices[, "y"],
  -SplitsLFY$.plot$vertices[, "x"]
) # Rotate 90ºCW
# Clean both
SplitsLFY <- TipClean(SplitsLFY)
SplitsLFYAcro <- TipClean(SplitsLFY, acronym = T)
SplitsLFY_Tall <- TipClean(SplitsLFY_Tall)
SplitsLFY_TallAcro <- TipClean(SplitsLFY_Tall, acronym = T)

# WXY data
SplitsWXY <- read.nexus.networx("/mnt/Results/PHYLO/WXY/WXY.splits.nex")
SplitsWXY <- TipClean(SplitsWXY)
SplitsWXYAcro <- TipClean(SplitsWXY, acronym = T)

# Nuclear Ambiguous data
SplitsNuclear <- read.nexus.networx("/mnt/Results/PHYLO/nuclear/Nuclear_ambig.fasta.splits.nex")
SplitsNuclearAcro <- TipClean(SplitsNuclear, acronym = T)

# All Ambiguous Data
SplitsAll <- read.nexus.networx("/mnt/Results/PHYLO/concatenated/TESTconcatented.splits.nex")
...
# Figure 3 with tall LFY plot (better than wide, I think, so wide is the Alt)
Figure3 %<a-% {
  par(mar = c(1, 0, 1, 0))
  split.screen(c(1, 2))
  par(oma = c(0, 0, 2, 0))
  ...
  splits.All
  screen(1) # maybe switch to screen 3
  splits.LFY.tall
...
# Save Figure 3
cairo_pdf(file = "/mnt/Results/Manuscript/Figure_3.pdf", height = 11, width = 15)
Figure3
dev.off()

# Figure 3 with wide LFY Plot
Figure3 %<a-% {
  par(mar = c(1, 0, 1, 0))
  split.screen(c(1, 2))
  par(oma = c(0, 0, 2, 0))
...
# Save Figure 3
cairo_pdf(file = "/mnt/Results/Manuscript/Figure_3_Alt.pdf", height = 11, width = 15)
Figure3
dev.off()

# Morphological Table -----------------------------------------------------
morpho <- read.csv2("/mnt/References/Morpho.csv", header = T, sep = ",")
...
# # Save in two formats
# # This won't work on my M1 Mac yet
# gtsave(morpho_table,
#        filename = "/mnt/Results/Mauscript/Morpho_table.pdf")
# gtsave(morpho_table,
#        filename = "/mnt/Results/Mauscript/Morpho_table.png")

# Supplemental Phylogenetic Trees -----------------------------------------
# read in the ambiguous, unphased, concatenated RAxML tree
# RAxMLPhylo <- phytools::read.newick("/mnt/Results/PHYLO/concatenated/GTRGI/TESTconcatenated.phy.partitioned.raxml.support")
# or read in tree collapsed by <10% bootstrap in Tree Graph
RAxMLPhylo <- phytools::read.newick("/mnt/Results/PHYLO/concatenated/GTRGI/TESTconcatenated.phy.partitioned.raxml.collapsed.nwk")
...
pdf(
  file = "/mnt/Results/Manuscript/Supplemental_1_Tree_concatenated_RAxML_GTRGI.pdf",
  width = 10,
  height = 22
)
plot(RAxMLPhylo,
